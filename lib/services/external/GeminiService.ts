...
`;
  ...
  /**
   * Dynamically rewrites campaign signal messages to be engaging and high-converting
   */
  async rewriteSignalMessages(signals: any[]): Promise<any[]> {
    ...
    try {
      const result = await this.executeWithTimeout(async () => {
        ...
        const systemPrompt = `You are an expert copywriter for WhatsApp marketing campaigns.
I will provide a list of campaign signals generated by our intelligence engine.
For each signal, rewrite the "original_message" to be extremely engaging, using emojis, high-converting language, and keeping it under 160 characters if possible.
Keep any links as exactly "[Link]". The output array MUST have the exact same length and order as the input signals.
...
Output ONLY a valid JSON array of strings containing the rewritten messages.`;
        ...
        const generatedText = response.text;
        if (!generatedText) return signals;

        try {
          const messages = JSON.parse(generatedText);
          if (Array.isArray(messages) && messages.length === signals.length) {
            return signals.map((s, idx) => ({
              ...s,
              suggested_message: messages[idx] || s.suggested_message
            }));
          }
        } catch {
          // ignore parse errors
