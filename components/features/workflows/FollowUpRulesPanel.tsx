...
import { useFollowUpRules } from '../../../lib/hooks';

export function FollowUpRulesPanel() {
  const { rules, loading, updateRule, reload } = useFollowUpRules();
  ...
  return (
    <div className="p-6">
      ...
      {showCreateModal && (
        <CreateRuleModal onClose={() => setShowCreateModal(false)} onSuccess={reload} />
      )}
    </div>
  );
}
...
function CreateRuleModal({
  ...
  const { createRule } = useFollowUpRules();
