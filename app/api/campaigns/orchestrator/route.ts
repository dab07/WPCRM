...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'process_scheduled') {
      ...
      // Temporarily return mock data to test the endpoint
      return NextResponse.json({
        ...
        source,
        message: 'Mock response - orchestrator endpoint working'
      });
    }
    ...
    return NextResponse.json({
      success: true,
      message: 'Single campaign execution not implemented in test mode'
    });
...
export async function GET() {
  return NextResponse.json({
    success: true,
    message: 'Campaign orchestrator endpoint is working',
    timestamp: new Date().toISOString()
  });
}
